...
class DailyReport extends CI_Controller
{
	...
	function daily_sale_report(){
	    ...
	    if ($where !='') { 
	      $this->db->where($where);
	        ...
	        $mail_message = "<html>
	        					<body>
	        						<p>Download store sales report.</p>
	        						<p>Date : ".date("d-m-yy")."</p>
	        							<a href='".base_url()."../uploads/report/".$fileName."'>click here</a>
	        					</body>
	        				</html>";
	        $data = $this->EmailModel->send_mail($mail_to,$mail_subject,$mail_message);
	        echo $data;
	}
	...
	function dailyPurchaseReport(){
	    ...
	    */
		...
		foreach ($result as $key => $value) {
	       ...
	       $mail_message = "<html>
	        					<body>
	        						<p>Download store purchase report.</p>
	        						<p>Date : ".date("d-m-yy")."</p>
	        							<a href='".base_url()."../uploads/report/".$fileName."'>click here</a>
	        					</body>
	        				</html>";
	...
	function warehousedaily_sale_report(){
	    ...
	    */
	    if ($where !='') { 
	      $this->db->where($where);
	        ...
	        $mail_message = "<html>
	        					<body>
	        						<p>Download warehouse sales report.</p>
	        						<p>Date : ".date("d-m-yy")."</p>
	        							<a href='".base_url()."../uploads/report/".$fileName."'>click here</a>
	        					</body>
	        				</html>";
	...
	function warehousedailyPurchaseReport(){
 
	    $where ='';
	    // if ($this->input->post('duration') == 'today') {
	      $where=" where date(created_date) = CURDATE()";
	 /*   }else if($this->input->post('duration') == 'week'){
	      $where=" where date(created_date) BETWEEN CURDATE() - INTERVAL 7 DAY AND CURDATE() AND purchased_by = ".$warehouseId." AND role='store'";
	    }else if($this->input->post('duration') == 'month'){
	      $where=" where date(created_date) BETWEEN CURDATE() - INTERVAL 1 MONTH AND CURDATE() AND purchased_by = ".$warehouseId." AND role='store'";
	    }else if($this->input->post('duration') == 'year'){
	      $where=" where date(created_date) BETWEEN CURDATE() - INTERVAL 1 YEAR AND CURDATE() AND purchased_by = ".$warehouseId." AND role='store'";
	    }else if($this->input->post('duration') == 'between'){
	      $where=" where date(created_date) BETWEEN  '".$_POST['from']."' AND '".$_POST['to']."' AND purchased_by = ".$warehouseId." AND role='store'";
	    }*/
		...
		foreach ($result as $key => $value) {
	        ...
	        $mail_message = "<html>
	        					<body>
	        						<p>Download warehouse purchase report.</p>
	        						<p>Date : ".date("d-m-yy")."</p>
	        							<a href='".base_url()."../uploads/report/".$fileName."'>click here</a>
	        					</body>
	        				</html>";
