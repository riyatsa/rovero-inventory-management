...
class StoreUserModel extends CI_Model
{
	...
	function get_warehouse_users($id=''){
		$wid = $this->session->userdata('logged_in_store_user');
		if ($id !='') {
		$result  = $this->db->where('usersId',$id)
				...
				$result  = $this->db->where('soreId',$wid['soreId'])
							->order_by('usersId','DESC')
							->get('store_users')
							->result_array();			 
		}
	...
	function total_sales(){
		$user = $this->session->userdata('logged_in_store_user'); 
		$count = $this->db->query('SELECT SUM(total) AS total_sales FROM warehouse_sales_order WHERE purchased_by='.$user['soreId'])->row_array(); 
		return $count;
	}
}