...
class ProductListModel extends CI_Model
{
	...
	function get_store_product_details($warehouse_Id =''){
...
/*		if($warehouse_Id != ''){
		...
		// echo "soreId : ".$user['soreId'];
			$this->db->where('storeId',$user['soreId']);
			// $this->db->where('store_user_i',$user['usersId']);
		$data = $this->db->select("*")
				->from('store_products') 
	...
	function get_warehouse_product_value_by_barcode(){
		$user = $this->session->userdata('logged_in_store_user_user');
		$where = array('storeId'=>$user['soreId'],'barcode'=>$this->input->post('barcode'));
		$result = $this->db->where('barcode',$this->input->post('barcode'))
				   ->get('warehouse_products')
				   ->row_array();
	...
	function get_generated_bill($param){
		...
		$where = array('purchased_by'=>$storeId, 'sales_id'=>$param);
		$result = $this->db->where($where)
						->order_by('sales_id','DESC')
...
$bill_data = array();
	...
	function get_generated_store_bill($param){
		...
		*/
		$user = $this->session->userdata('logged_in_store_user');
		$storeId = $user['soreId'];
		$where = array('purchased_by'=>$storeId, 'wareshouse_id'=>$param);
		$result = $this->db->where($where)
				->select('store_sales_order.*,store_customer.refral_code')
		...
		foreach ($product_id as $key => $prId) { 
			if ($prId !='' && $prId !='0') { 
				...
				$row['product_qty'] = (string)$product_qty[$key];
				$row['product_price'] = (string)$product_price[$key]; 
				$row['product_amount'] = (string)$product_qty[$key]*$product_price[$key]+$tax_price[$key];
				$row['tax_persent'] = $tax_persent[$key];
				$row['tax_price'] = $tax_price[$key];
				$qty += $product_qty[$key];
				$total += $product_price[$key];
				$amount_price += $amount[$key];
				$grand_total += $product_qty[$key]*$product_price[$key]+$tax_price[$key];
				$tax_price_total += $tax_price[$key];
				$tax_persent_total += $tax_persent[$key];
				array_push($bill_data, $row); 
			}
		}
