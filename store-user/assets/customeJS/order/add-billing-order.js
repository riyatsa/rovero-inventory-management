  ...
  $("#gst_price"+id).val(gst_price.toFixed(2));
   ...
   var subtotal = 0;
    if (applied_coupon_type == 1 && order_discounted_percentage !=0) { 
      // alert("price")
      subtotal = total_amounts.toFixed(2) - order_discounted_percentage.toFixed(2);
    }else if(applied_coupon_type == 0 && order_discounted_percentage !=0){ 
      // alert(order_discounted_percentage)
       subtotal_total = (total_amounts.toFixed(2) * order_discounted_percentage) / 100;
       subtotal = total_amounts.toFixed(2) - subtotal_total.toFixed(2);
    }else{ 
      // alert("no changes")
...
function apply_points(){
  var point = $("#customer_points").html();
  $('#customer_point_apply').val(parseFloat(point).toFixed(2));
  sumMyvalue();
}
