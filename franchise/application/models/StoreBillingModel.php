...
function add_new_item_for_billing(){
	return $this->db->where('store_product_id',$this->input->post('selected_item_product_id'))->get('store_products')->row_array();
}
...
function add_new_item_for_warehouse_billing(){
 ...
 function edit_store_sales_billing(){  
	...
	function insert_order(){ 
		...
		// $token = $this->SmsModal->get_sms_token();
		...
		if ($this->db->insert('store_sales_order',$order_data)) {
			...
			// echo $this->input->post('isCustomerNew') ;
	...
	function get_credit_history($num){
		$user = $this->session->userdata('logged_in_store');
 		$storeId = $user['storeId'];
		 $result = $this->db->where('customer_mobile',$num)//->where('storeId',$storeId)->where('role','store')
