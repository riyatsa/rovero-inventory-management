        ...
        for (var i = 0; i < data.length; i++) { 
          ...
          html+='<tr>'+
            ...
            '<td id="created_date_'+data[i].purchase_id+'">'+data[i].created_date+'</td>';
              if(data[i].approve_reject == '1'){
              html+='<td id="status_'+data[i].approve_reject+'" class="text-success">Confirm</td>'
            }else if(data[i].approve_reject == '0'){
              html+='<td id="status_'+data[i].approve_reject+'" class="text-danger">Cancel</td>'
            }else if(data[i].approve_reject == '2'){
              html+='<td id="status_'+data[i].approve_reject+'" class="text-warning">Pending</td>'
            }else{
               html+='<td id="status_'+data[i].approve_reject+'" class="text-info">Updated</td>'
            }

            if(data[i].approve_reject == '2'){
               html+='<td><a href="#" id="edit_store_'+data[i].purchase_id+' " onclick="view_store('+data[i].purchase_id+')" class="edit-a"><i class="fa fa-eye"></i></a><a href="#" id="edit_store_'+data[i].purchase_id+' " onclick="edit_store_order('+data[i].purchase_id+')" class="edit-a"><i class="fa fa-pencil"></i></a></td>'
            }else{
               html+='<td><a href="#" id="edit_store_'+data[i].purchase_id+' " onclick="view_store('+data[i].purchase_id+')" class="edit-a"><i class="fa fa-eye"></i></a><a download href="?/SaleReport/purchaseReport/'+data[i].purchase_id+'" ><i class="fa fa-download"></i></a></td>' 
            }
...
function edit_store_order(id){
 window.location.assign("?/edit-purchase-bill/"+id);
}
  ...
  $.ajax({
    ...
    success : function(data){
      ...
      let html_tr ='';
        ...
        html +='<td style="font-family:Arial, Helvetica, sans-serif; font-size:14px; font-weight:bold; color:#FFF; padding:12px 7px 12px 12px; background:#fa423b;">MRP</td>';
        ...
        html_tr +='<td style="font-family:Arial, Helvetica, sans-serif; font-size:14px; color:#000; padding:7px 7px 7px 12px; border-bottom:0px solid #000;">₹ '+data.product[i].product_mrp+'</td>';
