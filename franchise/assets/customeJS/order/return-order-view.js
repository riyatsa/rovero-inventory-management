...
function get_warehouse_product() {
	$.ajax({
    type  : 'ajax',
    url   : '?/returnOrder/get_return__order_list',
    async : false,
    dataType : 'json',
...
function view_store(id){
  ...
  $.ajax({
    type  : 'ajax',
    url   : '?/StoreOrder/get_store_return_purchase_order/'+id,
    async : false,
    dataType : 'json',
    success : function(data){
      let html = '';
      let html_tr ='';
        ...
        html +='</tr>';
        html +='<tr>';
        html +='<td align="center" style="font-family:Arial, Helvetica, sans-serif; font-size:30px; line-height:70px; color:#fa423b; font-weight:bold;">Return Purchase Order</td>';
        html +='</tr>';
        html +='<tr>';
        ...
        // html_tr +='<td style="font-family:Arial, Helvetica, sans-serif; font-size:14px; color:#000; padding:7px 7px 7px 12px; border-bottom:0px solid #000;">'+data.product[i].unit_name+'</td>';
