...
function get_all_item_list() { 
  $.ajax({
    type: "ajax",
    url: base_url+"?/StoreBilling/get_all_item_list",
    dataType: "json",
    ...
    if (jQuery.inArray(selected_item_product_id, order_product_id_array) === -1) {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          html += '<td>';
          html += '<div class="form-group">';
          html += '<input class="form-control tbl-fld ptotal_qty"  readonly  type="text" id="pqty'+add_new_item_i+'" onkeyup="sumMyvalue('+add_new_item_i+')" value="'+data.quantity+'">';
          html += '</div>';
          html += '</td>';
...
$('#paid_price').on('keyup blur',function() {
  var paid_price = $(this).val();
  var main_total_amount = $('#main_total_amount').val();
  if (paid_price != '') {
    if (!isNaN(paid_price)) {
      if (parseFloat(paid_price) <= parseFloat(main_total_amount)) {
        $('#paid-warnign-msg').html('');
        $("#balance_total").html((parseFloat(main_total_amount) - parseFloat(paid_price)).toFixed(2));
      } else {
        $('#paid_price').val(paid_price.slice(0,-1));
        $('#paid-warnign-msg').html('<span class="text-danger error-msg-small">Enterd amount should not be more then total amount.</span>');
      }
    } else {
      $('#paid_price').val(paid_price.slice(0,-1));
      $('#paid-warnign-msg').html('<span class="text-danger error-msg-small">Enterd amount should be only numbers.</span>');
    }
  } else {
    $('#paid-warnign-msg').html('<span class="text-danger error-msg-small">Please enter the pay amount.</span>');
  }
});
