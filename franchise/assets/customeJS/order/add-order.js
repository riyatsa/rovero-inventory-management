...
function save_product_order_confirm(){
     ...
     var paid_price = $("#paid_price").val();
     if (main_total_amount > 0 && paid_price > 0) {
      $("#order_btns").html('<button class="btn btn-default btn-close btn-add-2 ml-2" id="cancel_add_new_product_btn"  data-dismiss="modal">Cancel</button>                  <button class="btn btn-add btn-add-2 text-white mt-0" onclick="order_confirm()" id="add_new_product_btn" name="add_new_product_btn">Confirm</button>');
...
function order_confirm(){
   // $("#order_btns").html();
    $("#order_btns").html('<div class="spinner-grow text-success"></div> please wait..');
  save_product_order();
}