...
function save_warehouse_user(){
  ...
  $('#werehouse-error').html('<span class="d-block text-warning error-msg">Please wait while we are adding store User.</span>');
  ...
  $.ajax({
      type: "POST",
        url: base_url+"?/StoreUser/insert_store_users/",
        data:formdata,
        dataType: 'json',
        contentType: false,
        processData: false,
        success: function(data) {
          ...
          toastr.error('Something went wrong while adding store Users. Please try again.');
...
function edit_werehouse(id){
    ...
    $.ajax({
      type  : 'ajax',
      url   : '?/StoreUser/get_store_single_user/'+id,
      async : false,
      dataType : 'json',
      success : function(data){  
         ...
         // $('#edit-w-password').val();
           ...
           var usersId = '';
           if (data.role == 'add_product_user') {
                    usersId +='<option value="">Select Role</option>';
                    usersId +='<option selected value="add_product_user">Product User</option>';
                    usersId +='<option value="salesuser">sales User</option>';
           }else if(data.role == 'salesuser'){
                    usersId +='<option value="">Select Role</option>';
                    usersId +='<option value="add_product_user">Product User</option>';
                    usersId +='<option selected value="salesuser">sales User</option>';
           }else {
                    usersId +='<option selected value="">Select Role</option>';
                    usersId +='<option value="add_product_user">Product User</option>';
                    usersId +='<option value="salesuser">sales User</option>';
           }
...
function edit_update_warehouse_btn(){
  ...
  $.ajax({
      type: "POST",
        url: base_url+"?/StoreUser/update_store_users/"+usersId,
        data:formdata,
        dataType: 'json',
        contentType: false,
        processData: false,
        success: function(data) {
          if (data.status == '1') { 
            ...
            $('#w-password').val('');
             ...
             $('#edit-warehouse-users').modal('hide');
...
function users_status(id,status){
  ...
  $.ajax({
    type: "POST",
    url: base_url+"?/StoreUser/update_store_status/"+id,
