...
$('#vendorname').on('keyup blur',function(){
	var vendorname = $('#vendorname').val(); 
	if (vendorname != '') {
		$('#vendorname-error-msg-div').html('');
	} else {
		$('#vendorname-error-msg-div').html('<span class="text-danger error-msg-small">Please fill the vendor name.</span>');
	}
}); 
...
$('#email').on('keyup blur',function(){
	...
	if (email != '') {
		...
		$('#email-error-msg-div').html('<span class="text-danger error-msg-small">Please fill the email Name.</span>');
	}
});
...
function savevendorDetails(){
	...
	$('#cancelvendorDetailsbtn').prop('disabled',true);
	$('#savevendorDetailsbtn').prop('disabled',true);
	$('#savevendorDetailsbtn').css('background','#b3b3b3');
	$('#vendor-error').html('<span class="d-block text-warning error-msg">Please wait while we are adding vendor.</span>');
	...
	formdata.append('pincode',pincode);		
		...
		$.ajax({
		    ...
		    processData: false,
		  	success: function(data) {
		  		...
		  		$('#cancelvendorDetailsbtn').prop('disabled',false);
		  		$('#savevendorDetailsbtn').prop('disabled',false);
		  		$('#savevendorDetailsbtn').css('background','#1D3327');
		  		$('#vendor-error').html('');
		  	}
		});
		...
		if (vendorname != '') {
				...
				$('#vendorname-error-msg-div').html('<span class="text-danger error-msg-small">Please fill the werehouse name.</span>');
			}
		if (email != '') {
				$('#email-error-msg-div').html('');
			} else {
				$('#email-error-msg-div').html('<span class="text-danger error-msg-small">Please fill the User Name.</span>');
			}
		// if (password != '') {
		// 		$('#password-error-msg-div').html('');
		// 	} else {
