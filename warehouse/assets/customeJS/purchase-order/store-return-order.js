...
function get_warehouse_product() {
    $.ajax({
    type  : 'ajax',
    url   : '?/PurchaseOrder/get_return_store_order_list',
    async : false,
    dataType : 'json',
...
function view_store(id){
  ...
  $.ajax({
    type  : 'ajax',
    url   : '?/PurchaseOrder/get_return_purchase_order/'+id,
    async : false,
    dataType : 'json',
    success : function(data){ 
      // console.log(JSON.stringify(data))
...
function purchaseOrderRejectApproverd(id,status){
    ...
    $.ajax({
        type  : 'POST',
        url   : '?/PurchaseOrder/change_return_order_status/'+id, 
        data  : formdata,
        dataType: 'json',
        contentType: false,
        processData: false,
        success : function(data){
            if(data.status == 1){  
                ...
                if(status == 1){
                    ...
                    toastr.success('Return Order Accepted');
                    // window.location = "?/store-purchase-order/"+id;
                }else{
                    toastr.success('Return Order Rejeted');
                }
